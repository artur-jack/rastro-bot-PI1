...
'''
# Nova rota para buscar a primeira tupla da tabela corridas
@app.route('/api/first_corrida', methods=['GET'])
def get_first_corrida():
    connection = pymysql.connect(**db_config)
    cursor = connection.cursor()
    try:
        cursor.execute('SELECT * FROM corridas ORDER BY id ASC LIMIT 1')
        first_corrida = cursor.fetchone()
        if first_corrida:
            keys = [desc[0] for desc in cursor.description]
            first_corrida_dict = dict(zip(keys, first_corrida))
            return json_response(data=first_corrida_dict)
        else:
            return json_response(status=404, message='Nenhuma corrida encontrada.')
    except Exception as e:
        print('Erro ao buscar primeira corrida:', e)
        return json_response(status=500, message='Erro ao buscar primeira corrida.')
    finally:
        cursor.close()
        connection.close()
'''
