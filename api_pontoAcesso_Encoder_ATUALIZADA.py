...
def insert_data(data):
    ...
    try:
        ...
        wheel_diameter = 0.06  # Diâmetro da roda em metros (ajustar conforme necessário)
        ...
        if 'timeDIR' not in data or 'timeESQ' not in data or 'current' not in data:
        ...
        # Consumo calculado a partir do sensor de corrente
        consumption = data['current'] * (data['timeDIR'] / 1000.0)  # Consumo em Amperes-segundo
...
# Função para iniciar uma corrida automaticamente
@app.route('/api/auto_start_corrida', methods=['POST'])
def auto_start_corrida():
    ...
    return json_response(message='Corrida iniciada automaticamente!', corrida_id=corrida_id)

# Função para finalizar uma corrida
@app.route('/api/auto_end_corrida', methods=['POST'])
def auto_end_corrida():
    ...
    return json_response(message='Corrida finalizada automaticamente!', corrida_id=corrida_id)
...
def api_data():
    ...
    try:
        ...
        return json_response(status=500, message=f'Erro ao processar dados recebidos: {e}')

# Função para buscar uma corrida específica pelo ID
...
# Rota para buscar os dados mais recentes em tempo real
